...
// Componentes auxiliares para as estrelas
...
const StarRow = ({ rating, size = 18 }) => (
    <div className="inline-flex items-center">
        {Array.from({ length: 5 }, (_, i) => i + 1).map((n) => (
            <Star key={n} filled={rating >= n} size={size} />
        ))}
    </div>
);
        ...
        <div className="flex items-center gap-1">
            {Array.from({ length: 5 }, (_, i) => i + 1).map((n) => (
                ...
                >
                    <FaStar
                        size={28}
                        className={(hovered ?? value) >= n ? 'text-yellow-400' : 'text-gray-400'}
                    />
                </button>
            ))}
        </div>
...
export default function NewsDetailClient({ noticia }) {
    ...
    useEffect(() => {
        ...
        const initialReviews = savedReviews.length > 0 ? savedReviews : NEWS_COMMENTS[noticia.id] || [];
    ...
    return (
        <main className="bg-gradient-to-br from-white via-purple-400 to-purple-950 dark:bg-gradient-to-br dark:from-black dark:via-purple-700 dark:to-purple-950 text-black dark:text-white p-4">
            <div className="max-w-7xl mx-auto py-8">

                <Link href="/news" className="flex items-center gap-2 mb-6 text-purple-950 dark:text-white hover:text-purple-600 dark:hover:text-purple-400 transition-colors">
                    <FaArrowLeft />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:items-start">
                    <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
                        <img src={noticia.imagem} alt={noticia.titulo} className="w-full h-80 object-cover" />
                        <div className="p-8">
                            <h1 className="text-4xl font-extrabold mb-4 text-purple-950 dark:text-white leading-tight">{noticia.titulo}</h1>
                            ...
                            {noticia.texto.split('\n').map((par, i, arr) => (
                                <p key={i} className={`text-lg text-gray-700 dark:text-gray-300 leading-relaxed ${i !== arr.length - 1 ? 'mb-6' : 'mb-0'}`}>{par}</p>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-1 space-y-8">
                        <section className="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-xl">
                            <h2 className="text-2xl font-bold mb-6 text-purple-950 dark:text-white">Deixe sua avaliação</h2>
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <label className="block">
                                    <span className="text-sm font-medium text-black dark:text-white">Sua nota</span>
                                    <StarRatingInput value={rating} onChange={setRating} />
                                </label>
                                <label className="block">
                                    <span className="text-sm font-medium text-black dark:text-white">Seu nome</span>
                                    <input
                                        type="text"
                                        value={name}
                                ...
                                <label className="block">
                                    <span className="text-sm font-medium text-black dark:text-white">Comentário</span>
                                    <textarea
                                        value={comment}
                        ...
                        <section className="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-xl">
                            <h3 className="text-2xl font-bold mb-6 text-purple-950 dark:text-white">Comentários ({reviews.length})</h3>
                            {reviews.length > 0 ? (
                                <ul className="space-y-6">
                                    {reviews.map((review) => (
                                        <li key={review.id} className="border-b border-gray-200 dark:border-gray-700 pb-6 last:border-b-0 last:pb-0">
                                ...
                                <p className="text-gray-500">Ninguém avaliou esta notícia ainda. Seja o primeiro!</p>
                            )}
                        </section>
                    </div>
                </div>
