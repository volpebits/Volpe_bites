...
const GOOGLE_CLIENT_ID = "156246719780-7ma9cl5ld9qc2c41sna02162jra5msrj.apps.googleusercontent.com";
...
const computeUserId = (email) =>
    String(email || "guest").trim().toLowerCase().replace(/[^\w.-]+/g, "_");

const readAuth = () => JSON.parse(localStorage.getItem(AUTH_KEY) || "null");
const writeAuth = (obj) => localStorage.setItem(AUTH_KEY, JSON.stringify(obj || null));
const readUsers = () => JSON.parse(localStorage.getItem(USERS_KEY) || "{}");
const writeUsers = (map) => localStorage.setItem(USERS_KEY, JSON.stringify(map || {}));
...
export default function SlidePanel({ isOpen, setIsOpen }) {
    ...
    useEffect(() => {
        ...
        if (!window.grecaptcha) {
            ...
            recaptchaScript.src = 'https://www.google.com/recaptcha/api.js?render=explicit&hl=pt-BR';
    ...
    const openReadyPlayerMe = () => {
        window.open('https://demo.readyplayer.me/avatar?frameApi', '_blank', 'width=400,height=600');
    };
    ...
    const handleGoogleCallback = async (response) => {
        try {
            ...
            if (!exists) {
                users[id] = makeUserRecord({ email, username: name, avatar, password: "" });
            } else {
    ...
    const handleBackToLogin = () => { setAuthMode('login'); setRecaptchaToken(null); };
    ...
    const getTitle = () => (isLogin ? 'Log in' : isRegister ? 'Crie uma conta' : 'Recuperar senha');
    const getButtonText = () => (isLogin ? 'Entrar' : isRegister ? 'Crie sua conta!' : 'Enviar link de recuperação');

    return (
        <>
            {isOpen && <div onClick={() => setIsOpen(false)} className="fixed inset-0 bg-black/30 backdrop-blur-sm z-40" />}

            <div className={`fixed inset-0 z-50 flex font-sans transition-transform duration-500 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="w-0 md:w-1/2 bg-black/30 backdrop-blur-sm" onClick={() => setIsOpen(false)} />
                <div className="relative w-full md:w-1/2 bg-white dark:bg-gray-900 h-screen flex flex-col px-6 py-16 shadow-xl overflow-y-auto">
                    <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 text-gray-500 hover:text-gray-700 transition">
                        <X size={20} />
                    </button>
                    ...
                    <div className="w-full max-w-md mx-auto mt-10 space-y-10">
                        <div className="flex items-center space-x-4">
                            {(isRegister || isForgotPassword) && (
                                <button onClick={handleBackToLogin} className="text-gray-500 hover:text-gray-700 transition">
                                    <ArrowLeft size={20} />
                                </button>
                            )}
                            <h2 className="text-3xl font-bold text-green-500">{getTitle()}</h2>
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="w-full max-w-md mx-auto mt-10 space-y-10">
                        <div className="space-y-6">
                            ...
                            {isRegister && (
                                <div className="text-center space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <button type="button" onClick={() => window.open('https://demo.readyplayer.me/avatar?frameApi', '_blank', 'width=400,height=600')} className="flex items-center justify-center gap-2 px-4 py-3 bg-green-500 hover:bg-green-600 text-white rounded-lg text-sm transition-colors">
                                            <Camera size={16} /> 3D Avatar
                                        </button>

                                        <button type="button" onClick={generateAIAvatar} disabled={isGeneratingAI} className="flex items-center justify-center gap-2 px-4 py-3 bg-purple-500 hover:bg-purple-600 disabled:bg-purple-300 text-white rounded-lg text-sm transition-colors">
                                            {isGeneratingAI ? <Loader2 size={16} className="animate-spin" /> : <Wand2 size={16} />} IA Random
                                        </button>
                                    </div>
                                </div>
                            )}
                            ...
                            {/* E-mail */}
                            <div>
                                <label className="block text-sm text-gray-600 dark:text-white mb-1">Endereço de e-mail</label>
                                <input
                                    type="email"
                                ...
                                <div>
                                    ...
                                    {isLogin && (
                                        <div className="w-full flex justify-between items-center mt-2">
                                            ...
                                            <button type="button" onClick={handleForgotPassword} className="text-green-500 hover:underline transition-all duration-200 text-sm">
                                                Esqueceu sua senha?
                                            </button>
                                        </div>
                                    )}
                            ...
                            {!isForgotPassword && (
                                ...
                                >
                                    ...
                                    {isGoogleLoaded ? (isLogin ? 'Entrar com Google' : 'Cadastrar com Google') : 'Carregando Google...'}
                                </button>
                            )}
                            ...
                            {isForgotPassword && (
                                <div className="text-center">
                                    <p className="text-sm text-gray-600 dark:text-white">
                                        ...
                                        <button type="button" onClick={handleBackToLogin} className="text-green-500 hover:underline transition-all duration-200">
                                            Voltar ao login
                                        </button>
                                    </p>
                                </div>
