...
const missionTemplates = [
  {
    type: "play",
    title: "Maratonista",
    description: "Jogue por {time} consecutivas",
    icons: ["⏰", "🎮", "⚡"],
    timeValues: ["1 hora", "2 horas", "3 horas"],
  },
  {
    type: "games",
    title: "Explorador",
    description: "Complete {count} jogos diferentes",
    icons: ["🗺️", "🎯", "🌟"],
    countValues: [2, 3, 4, 5],
  },
  {
    type: "genre",
    title: "Especialista",
    description: "Jogue 3 jogos de {genre}",
    icons: ["🎭", "⚔️", "🏎️"],
    genres: ["RPG", "Aventura", "Corrida", "Estratégia", "Indie"],
  },
  {
    type: "social",
    title: "Avaliador",
    description: "Avalie {count} jogos",
    icons: ["⭐", "📝", "👍"],
    countValues: [2, 3, 4],
  },
  {
    ...
    description: "Compartilhe {count} jogos com amigos",
    icons: ["📤", "🤝", "💫"],
    countValues: [1, 2, 3],
  },
  {
    type: "social",
    title: "Crítico",
    description: "Escreva uma review de {count} jogos",
    icons: ["✍️", "📖", "🎬"],
    countValues: [1, 2],
  },
  {
    type: "achievement",
    title: "Colecionador",
    description: "Desbloqueie {count} conquistas",
    icons: ["🏆", "💎", "🎗️"],
    countValues: [2, 3, 4, 5],
  },
  {
    ...
    description: "Complete um jogo 100%",
    icons: ["💯", "👑", "✨"],
    countValues: [1],
  },
  {
    type: "achievement",
    title: "Speedrunner",
    description: "Complete um jogo em menos de {time}",
    icons: ["🏃", "⚡", "🕐"],
    timeValues: ["1 hora", "30 minutos", "2 horas"],
  },
  {
    type: "discovery",
    title: "Descobridor",
    description: "Encontre {count} jogos ocultos",
    icons: ["🔍", "🗝️", "🎁"],
    countValues: [1, 2, 3],
  },
  {
    type: "discovery",
    title: "Beta Tester",
    description: "Teste {count} jogos em desenvolvimento",
    icons: ["🧪", "⚗️", "🔬"],
    countValues: [1, 2],
  },
  {
    type: "special",
    title: "Nostálgico",
    description: "Jogue um jogo clássico brasileiro",
    icons: ["🎖️", "📼", "🕹️"],
    countValues: [1],
  },
  {
    type: "special",
    title: "Patriota",
    description: "Complete {count} jogos nacionais",
    icons: ["🇧🇷", "🎊", "🏆"],
    countValues: [2, 3, 4],
  },
  {
...
function resolveDescription(template) {
  ...
  if (template.countValues && description.includes("{count}")) {
    const count =
      template.countValues[
        Math.floor(Math.random() * template.countValues.length)
      ];
  ...
  if (template.timeValues && description.includes("{time}")) {
    const time =
      template.timeValues[
        Math.floor(Math.random() * template.timeValues.length)
      ];
    description = description.replace("{time}", time);
  }
  if (template.genres && description.includes("{genre}")) {
    const genre =
      template.genres[Math.floor(Math.random() * template.genres.length)];
...
function generateDailyMissionsExactXP(neededXP, maxMissions = 5) {
  ...
  for (let i = 0; i < maxMissions && remaining > 0; i++) {
    ...
    const icon =
      template.icons[Math.floor(Math.random() * template.icons.length)];
...
const UserProfilePage = () => {
  ...
  const handleSaveProfile = () => {
    setProfileData((prev) => {
      ...
      if (
        !onboarding.profileConfigured &&
        (next.name || next.about || next.avatar)
      ) {
  ...
  // Missões
  const [missionsFinishedToday, setMissionsFinishedToday] =
  ...
  const regenerateSpecificMission = (missionId) => {
    ...
    const xpForNew =
      neededForThis > 0
        ...
        : Math.max(
            10,
            dailyMissions.find((m) => m.id === missionId)?.xpReward ?? 10
          );
    ...
    setDailyMissions((prev) =>
      prev.map((m) => (m.id === missionId ? newMission : m))
    );
  };
  ...
  if (!mounted || !profileData) {
    return (
      <div className="min-h-screen p-4 bg-gradient-to-br from-white via-purple-400 to-purple-950 dark:bg-gradient-to-br dark:from-black dark:via-purple-700 dark:to-purple-950">
        <div className="max-w-7xl mx-auto animate-pulse text-white/70">
          Carregando…
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-4 gap-6 mb-8">
          <div className="bg-purple-900/30 backdrop-blur-sm rounded-xl p-6 border border-purple-900/80">
            ...
            <div className="w-full bg-gray-700 rounded-full h-3">
              <div
                ...
                style={{
                  width: `${
                    (profileData.currentXP / profileData.maxXP) * 100
                  }%`,
                }}
              />
            </div>
          ...
          <div className="bg-purple-900/30 backdrop-blur-sm rounded-xl p-6 border border-purple-900/80 text-center">
            ...
            <Gamepad2
              className="w-8 h-8 text-purple-900 dark:text-purple-500 font-bold mx-auto"
              aria-hidden
            />
          </div>
          ...
          <div className="bg-purple-900/30 backdrop-blur-sm rounded-xl p-6 border border-purple-900/80 text-center">
            ...
            <Clock
              className="w-8 h-8 text-green-500 font-bold mx-auto"
              aria-hidden
            />
          </div>
        </div>
                ...
                <div className="bg-purple-900/30 backdrop-blur-sm rounded-xl p-6 border border-purple-900/80">
                  ...
                  <p className="text-sm text-white/80 mb-2">
                    ...
                    <span className="font-bold text-green-300">
                      {Math.max(0, profileData.maxXP - profileData.currentXP)}{" "}
                      XP
                    </span>{" "}
                ...
                <div>
                  ...
                  {profileData.favoriteGenres?.length ? (
                    ...
                    <p className="text-white/70 mb-3">
                      Nenhum gênero adicionado ainda. Comece adicionando os que
                      você mais curte!
                    </p>
                  )}
                  ...
                  <div className="bg-purple-800/20 rounded-lg p-6 border border-purple-900/80">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-black dark:text-white text-sm">
                        {profileData.currentXP}/{profileData.maxXP} XP para o
                        próximo nível
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-green-500/20 border border-green-500/50 rounded-lg p-4">
                    ...
                    <p className="text-green-300 text-sm mb-4">
                      Jogue seu primeiro jogo para ganhar XP e começar sua
                      jornada
                    </p>
                    <button
                      onClick={() => {
                  ...
                  <div className="bg-yellow-500/20 border border-yellow-500/50 rounded-lg p-4">
                    ...
                    <p className="text-yellow-300 text-sm mb-4">
                      Assim que as missões forem desbloqueadas, conclua uma para
                      sua 1ª conquista
                    </p>
                    <button
                      disabled
                  ...
                  <p className="text-xs text-white/70">
                    As missões desbloqueiam automaticamente quando concluir os 2
                    passos acima.
                  </p>
                </div>
              </>
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() =>
                              regenerateSpecificMission(mission.id)
                            }
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <p className="text-xs text-black/60 dark:text-white/60">
                  Dica: senha deve ser tratada no backend; não a salve em
                  localStorage.
                </p>
              </div>
